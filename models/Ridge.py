...
def all_one_zeros(data):
    if (len(np.unique(data)) == 2):
        ...
        flag = True
    return flag
# file = '../dataset/TYK2/TYK2_reg_clu.csv'
...
def best_model_running(seed,best_results,data,split_type='random',FP_type='ECFP4',task_type = 'cla',model_dir=False):
    pd_res = []
    if task_type == 'cla':
        while True:
    ...
    model = Ridge(alpha=best_results['alpha'], normalize=True, max_iter=10000, random_state=0)

    model.fit(data_tr_x, data_tr_y)
    if model_dir:
        model_name = str(model_dir) + '/%s_%s_%s_%s_%s' % (split_type, task_type, FP_type, seed, 'Ridge_bestModel.pkl')
...
def run_ridge(X,Y,split_type='random',FP_type='ECFP4',task_type = 'cla',model_dir=False):
    ...
    pd_res = []
    OPT_ITERS = 50
    space_ = {'alpha': hp.uniform('alpha', 0.01, 100)}

    def hyper_opt(args):
        model = Ridge(**args, normalize=True, max_iter=10000, random_state=0)
    ...
    split_type, task_type, FP_type, 'Ridge.param')
    ...
    best_model = Ridge(alpha=best_results['alpha'], normalize=True, max_iter=10000, random_state=0)
    best_model.fit(data_tr_x, data_tr_y)
    if model_dir:
        model_name = str(model_dir) + '/%s_%s_%s_%s' % (split_type, task_type, FP_type, 'Ridge_bestModel.pkl')
        joblib.dump(best_model, model_name)
    num_of_compounds = len(X)
    if task_type == 'cla':
        tr_pred = best_model.predict_proba(data_tr_x)
        tr_results = [FP_type, split_type, 'tr', num_of_compounds, best_results['alpha']]
        tr_results.extend(statistical(data_tr_y, np.argmax(tr_pred, axis=1), tr_pred[:, 1]))
        pd_res.append(tr_results)
        # validation set
        va_pred = best_model.predict_proba(data_va_x)
        va_results = [FP_type, split_type, 'va', num_of_compounds, best_results['alpha']]
        va_results.extend(statistical(data_va_y, np.argmax(va_pred, axis=1), va_pred[:, 1]))
        pd_res.append(va_results)
        # test set
        te_pred = best_model.predict_proba(data_te_x)
        te_results = [FP_type, split_type, 'te', num_of_compounds ,best_results['alpha']]
        te_results.extend(statistical(data_te_y, np.argmax(te_pred, axis=1), te_pred[:, 1]))
        pd_res.append(te_results)
    ...
    para_name, best_name = os.path.join(result_dir,
                                        '_'.join([split_type, 'Ridge', FP_type, 'para.csv'])), os.path.join(
        result_dir, '_'.join([split_type, 'Ridge', FP_type, 'best.csv']))
    para_res.to_csv(para_name, index=False)
    best_res.to_csv(best_name, index=False)

    return para_res, best_res

# para,best = tvt_svm(X,Y,split_type='cluster',FP_type='ECFP4',task_type='reg')
# # print(para)
# print(best)
