...
def best_model_running(seed,best_hyper,data,split_type='random',FP_type='ECFP4',task_type = 'cla',model_dir=False):
    ...
    model = LGBMRegressor(max_depth=best_hyper['max_depth'],
                          learning_rate=best_hyper['learning_rate'],
                          subsample=best_hyper['subsample'],
                          boosting_type='gbdt',
                          num_leaves=best_hyper['num_leaves'], n_estimators=100, objective='regression',
                          colsample_bytree=best_hyper['colsample_bytree'],
                          min_child_samples=20,
                          reg_alpha=best_hyper['reg_alpha'], reg_lambda=best_hyper['reg_lambda'],
                          random_state=1) \
        if task_type == 'reg' else None

    model.fit(data_tr_x, data_tr_y)
    if model_dir:
        model_name = str(model_dir) + '/%s_%s_%s_%s_%s' % (split_type, task_type, FP_type, seed, 'LGBM_bestModel.pkl')
...
def run_lgbm(X,Y,split_type='random',FP_type='ECFP4',task_type='cla',model_dir=False):
    random_state = 20
    data = TVT(X, Y)
    ...
    space_ = {'learning_rate': hp.uniform('learning_rate', 0.01, 0.1),
              'max_depth': hp.randint('max_depth', 3, 25),
              'num_leaves': hp.randint('num_leaves', 5, 31),
              'colsample_bytree': hp.uniformint('colsample_bytree', 0.6, 1.0),
              'subsample': hp.uniformint('subsample', 0.6, 1.0),
              'reg_alpha': hp.loguniform('reg_alpha', np.log(0.01), np.log(1000)),
              'reg_lambda': hp.loguniform('reg_lambda', np.log(0.01), np.log(1000))
    }

    def hyper_opt(args):
        model = LGBMRegressor(**args, boosting_type='gbdt', n_estimators=100, objective='regression',
                              min_child_samples=20, random_state=1, silent=True, n_jobs=50)
        model.fit(data_tr_x, data_tr_y)
        val_preds = model.predict(data_va_x)
    ...
    para_file = str(model_dir).replace('model_save', 'param_save') + '/%s_%s_%s_%s' % (
        split_type, task_type, FP_type, 'LGBM.param')
    ...
    best_model = LGBMRegressor(learning_rate=best_results['learning_rate'],
                               max_depth=best_results['max_depth'],
                               subsample=best_results['subsample'], boosting_type='gbdt',
                               num_leaves=best_results['num_leaves'], n_estimators=100, objective='regression',
                               colsample_bytree=best_results['colsample_bytree'],
                               min_child_samples=20,
                               reg_alpha=best_results['reg_alpha'], reg_lambda=best_results['reg_lambda'],
                               random_state=1, n_jobs=50)
    best_model.fit(data_tr_x, data_tr_y)
    if model_dir:
        model_name = str(model_dir) + '/%s_%s_%s_%s' % (split_type, task_type, FP_type, 'LGBM_bestModel.pkl')
        joblib.dump(best_model, model_name)
    num_of_compounds = len(X)
    if task_type == 'cla':
        tr_pred = best_model.predict_proba(data_tr_x)
        tr_results = [FP_type, split_type, 'tr', num_of_compounds,
                      best_results['max_depth'], best_results['num_leaves'], best_results['learning_rate'], best_results['subsample']]
        tr_results.extend(statistical(data_tr_y, np.argmax(tr_pred, axis=1), tr_pred[:, 1]))
        pd_res.append(tr_results)
        # validation set
        ...
        para_res = pd.DataFrame(pd_res, columns=['FP_type', 'split_type', 'type', 'num_of_compounds',
                                                 'max_depth', 'num_leaves', 'learning_rate', 'subsample',
    ...
    para_name, best_name = os.path.join(result_dir,
                                        '_'.join([split_type, 'LGBM', FP_type, 'para.csv'])), os.path.join(
        result_dir, '_'.join([split_type, 'LGBM', FP_type, 'best.csv']))
...
# run_lgbm(X,Y,split_type='random',FP_type='ECFP4',task_type='reg', model_dir='/home/weili/zyh/')
# # print(para,best)
