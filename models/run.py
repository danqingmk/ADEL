...
def model_set(split_type='random',FP_type='ECFP4',model_type = 'SVM',model_dir=False,file_name=None,task_type='reg',difftasks=None):
    if model_type =='SVM':
...
def pair_param(file_name,data,split_type,model_type,FP_type,difftasks,iter):
    ...
    if model_type in ['SVM','KNN','RF','XGB','LGBM','Cubist','Ridge']:
        ...
        if os.path.exists(paramname):
            ...
            print(paramname, 'doing')
            model_set(split_type=split_type, model_type=model_type, model_dir=model_dir,
